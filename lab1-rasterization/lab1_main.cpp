...
void initialize()
{
    ...
    // See OpenGL Spec §2.9 Buffer Objects
    // - http://www.cse.chalmers.se/edu/course/TDA361/glspec30.20080923.pdf#page=54
    GLuint position2Buffer;
    glGenBuffers(1, &position2Buffer);
    // Set the newly created buffer as the current one
    glBindBuffer(GL_ARRAY_BUFFER, position2Buffer);
    // Send the vertex position data to the current buffer
    ...
    if (!vs_file.is_open())
    {
        labhelper::fatal_error(std::string("Failed to open shader file: ") + "../lab1-rasterization/simple.vert");
        return;
    }
    ...
    if (!fs_file.is_open())
    {
        labhelper::fatal_error(std::string("Failed to open shader file: ") + "../lab1-rasterization/simple.frag");
        return;
    }
    ...
    // vertex/fragment shaders,  do not appear before linking.
    {
        ...
        if (!linkOk)
        {
            std::string err = labhelper::GetShaderProgramInfoLog(shaderProgram);
            labhelper::fatal_error(err);
            return;
        }
    }
